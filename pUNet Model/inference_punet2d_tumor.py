...
from punet_modelbase import BLVnet2dModule
...
def inference():
    height = 512
    width = 512
   punet2d = BLVnet2dModule(512, 512, channels=1, costname=("dice coefficient",), inference=True,
    ...
    for subsetindex in range(len(path_list)):
        ...
        for z in range(index):
            if np.max(maskarray[z]) != 0:
                imagemask[z, :, :] = punet2d.prediction(imagearray[z])
