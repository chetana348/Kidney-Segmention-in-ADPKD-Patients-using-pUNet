...
for (lr_) in params:
    # Define Model################################################################################################
    punet = nnUNet(64, 5, use_xavier=True, use_batchNorm=True, dropout=0.5, retain_size=True, nbCls=2)
    ...
    if torch.cuda.is_available():
        ...
        if device_num > 1:
            punet = torch.nn.DataParallel(punet)
    punet.to(config.DEVICE)
    #############################################################################################################
    ...
    opt = torch.optim.NAdam(nnunet.parameters(), lr=lr_)
    ...
    for e in pbar:
        ...
        torch.save(punet.state_dict(), './final_result/punet_{}.pt'.format(e + 1))
    ...
    print('Saving model...\n\n')
    torch.save(punet.state_dict(), './final_result/PUNet.pt')
